...
func createSudoWrapperScript(tabID int) (string, func(), error) {
	server := ensureSudoIPCServer()
	if server == nil {
		return "", func() {}, fmt.Errorf("sudo IPC server not initialized")
	}
	...
	tmpFile, err := os.CreateTemp("", "ask-sudo-wrapper-*.sh")
	if err != nil {
		return "", func() {}, err
	}
...
export ASK_SUDO_TOKEN="%s"
export ASK_SUDO_SOCKET="%s"
export ASK_SUDO_TABID="%d"
exec "%s" "$@"
`, server.token, server.socketPath, tabID, askExe)
