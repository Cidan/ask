...
| `agent_provider.go`    | `agentAPIProvider` — the ONE adapter from a `providers.Provider` (pkg/providers/provider.go) to the TUI's `Provider` interface, registered once per registry entry in provider.go's `init()`: StartSession builds the model through `engine.ModelBuilder` up front (a provider that cannot run fails here with its own message), capability-gated image attachments, shared tool assembly (`setupAgentSessionTools`), settings via `providers.LoadSettings`/`SaveSettings`. `ModelPicker()` is synchronous and network-free (cached live listing from model_catalog.go, else `ModelOptions()`); `ListModels` type-asserts `providers.ModelLister` and is the network path the catalog load calls. |
| `pkg/providers/provider.go` | THE provider contract and registry. `Provider` is a Go interface — every method required (identity, `Settings()`, `Configured`, `BuildModel` → ADK `model.LLM`, `CanonicalModelID`, `CallOptions`, `SupportsImages`, `ContextWindow`, `MaxOutputTokens`); `ModelLister` is the one optional capability, found by type assertion. `SettingField{Key, Title, Hint, Secret, EnvKey, Default, Validate}` is how a provider declares its configuration — the `/config` screen, the model picker's key prompt, and `Configured` are all driven by it; `SettingValue` resolves stored → env → default. `builtin` is the registration order (first = default provider, `DefaultProviderID()`); `Register` panics on a malformed provider (empty id, duplicate/reserved setting key). `LoadSettings`/`SaveSettings` (model + slash commands per provider, effort global) and `ResolveModelID` (explicit → configured → default, canonicalized) are the shared accessors every caller uses instead of reaching into config. See "Adding a provider" below. |
| `pkg/providers/steering.go` | The coder steering prompt paragraphs (`SteeringPrompt`). |
| `pkg/providers/vertex.go` | `Vertex` — Gemini through ADK's `gemini.NewModel`, ADC or an explicit service-account key. Declares `project` (env `GOOGLE_CLOUD_PROJECT`), `location` (default `global`), `serviceAccountKey` (env `GOOGLE_APPLICATION_CREDENTIALS`, tilde-expanded) with their validators (`ValidateVertex*`); `VertexPrepareCredentials` / `VertexModel` / `ListVertexModels` are the swappable seams. Filters Claude ids out of the model list. |
| `pkg/providers/openrouter.go` | `OpenRouter` — the shared `openai_compat.go` model over openrouter.ai; declares `apiKey` (Secret, env `OPENROUTER_API_KEY`) and `baseURL` (default). Live `/models` metadata feeds capabilities and the reasoning encoder. |
| `config_fields.go`    | THE data-driven `/config` sub-picker for every "list of editable fields" screen: one `fieldsPickerState` on the model, opened with a `[]providers.SettingField` + load/save closures — `openProviderFieldsPicker(p)` for every registered provider (rows generated from `p.Settings()`, persisted into `cfg.Providers[id]`, the tab's live session restarted when it runs on that provider) and `openWebSearchFieldsPicker` for the Brave key. Rows show the stored value (masked for `Secret`), else `from $ENV`, else `<default> (default)`; the inline editor validates with `SettingField.Validate` and keeps itself open on failure. Renders through `renderLayeredConfigBox` like Project Options. Adding a provider adds no UI code. |
...
| `pkg/engine/subagents.go` | Named subagent defs: `.claude/agents/*.md` + `~/.config/ask/agents` (frontmatter name/description/tools/model + ask's `provider` extension; body = system prompt), `<available_agents>` prompt block, tool grant sets (default read-only, `*` = coding core, never task/modal tools). The task tool (agent_tools_task.go) runs a def on the parent session's provider/model unless the def pins `provider:` and/or `model:` — a pinned provider with no model runs that provider's configured or default model. |
...
| `config_fields_test.go`    | The data-driven `/config` field picker — one row per registered provider generated from the registry (`on`/`off` via `Configured`), Enter on a row opens that provider's picker and routes keys through `updateConfigModal`, Vertex edit end to end (rows in `Settings()` order, editor pre-fills the stored value, validation keeps the editor open and persists nothing, project/location/SA-key persist into `cfg.Providers["vertex"]`, clearing removes the key), secret masking in row + editor and the `from $ENV` summary, Web Search round-trip + clear through the same picker, PasteMsg routing, save restarts the live session only for the tab's own provider, layered-box view + filter. |
...
| `pkg/providers/provider_test.go` | The registry + contract — builtin order and lookup, `All` copies, `Register` replaces same-id in place and panics on malformed providers (nil, empty id, empty/duplicate/reserved setting key), builtin settings well-formed, `SecretField`/`FieldByKey`, `SettingValue` stored → env → default, `ProviderConfigured`, `LoadSettings`/`SaveSettings` keep declared fields, `ResolveModelID` (explicit → configured → default, real providers included), `expandTilde`. |
| `pkg/providers/vertex_test.go` | `Vertex` — identity/options/limits/images, foreign-id canonicalization, settings order, `Configured`, `BuildModel` fail-fast without a project, validators (project/location/SA-key incl. tilde + directory rejection), `VertexPrepareCredentials` (swapped loader, no env mutation, missing file, ADC nil, tilde expansion, env fallback), effort mapping, swapped `ListVertexModels` via `ModelLister`. |
| `pkg/providers/openrouter_test.go` | `OpenRouter` — key/base-URL resolution (config over env/default), `/models` listing + meta cache via `ModelLister`, `BuildModel` without/with a key, identity, slug pass-through, settings order, `Configured`. |
| `pkg/config/provider_config_test.go` | `ProviderConfig` — flat JSON round-trip (fields beside `model`, empties dropped, non-string extras dropped, malformed typed keys rejected), `WithField` copy semantics, `SetProviderConfig` map hygiene, on-disk `providers` shape, legacy `vertex`/`openrouter` block migration (serviceAccount alias, dropped keys, unregistered blocks ignored, new block wins, stable re-save). |
...
`Configured` method of `providers.Provider`) with a toast telling the
...
satisfies the TUI's `Provider` interface for every registry entry, with
`providerProc.cmd == nil`: StartSession builds the model through
`engine.ModelBuilder` (fail-fast: no project / no key surfaces here)
and spawns a goroutine (`agentSession.run`), `stdin` is an adapter whose
`Close()` tears the session down (that's what `killProc` calls), and
`Interrupt` cancels the in-flight turn's context cooperatively
(handled=true, codex-style — the session emits its own turn end). A
provider is a `providers.Provider` implementation
(`pkg/providers/provider.go`): identity, model/effort options,
`Settings()`, `Configured`, `BuildModel` (an ADK `model.LLM`),
`CanonicalModelID`, `CallOptions` (effort→wire), image capability,
context window, output budget. Registration order is explicit in
`providers.builtin`; cmd/ask's `init()` wraps every entry.

### Adding a provider

1. `pkg/providers/<name>.go`: a type implementing `Provider` (pin it
   with `var _ Provider = Name{}`), declaring its configuration as
   `[]SettingField` — the credential is the `Secret` field, env
   fallbacks go on `EnvKey`, defaults on `Default`, input checks on
   `Validate`. Implement `ModelLister` if the provider can enumerate
   its models. `BuildModel` reads its fields with `SettingValue`.
2. Add it to `builtin` in provider.go.

That is the whole change. The `/config → <Name>...` row and its field
editor (config_fields.go), the model picker section and its key prompt
(model_picker.go), `ProviderConfigured` for workflow steps, the cost
meter, sessions, and the headless engine all iterate the registry.
Config lands under `providers.<id>` in ask.json as a flat block
(`config.ProviderConfig`: `model`, `slashCommands`, and the declared
fields by key); nothing in `pkg/config` names a provider.
...
**Vertex AI** rides ADK's `gemini.NewModel` with a `genai.BackendVertexAI` client config. Auth uses Google Cloud ADC: the `serviceAccountKey` setting (or `$GOOGLE_APPLICATION_CREDENTIALS` env) → `gcloud auth application-default login` creds → GCE metadata. `VertexPrepareCredentials` validates the SA-key path (tilde-expanded) and uses `genai.ClientConfig{Credentials: ...}` directly instead of mutating the process environment. Project is required (fail-fast at session start); Location defaults to `global`. Default model is `gemini-3.7-flash`.
