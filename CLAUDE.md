...
| `agent_tools_file.go`  | read / write / edit tools (exact-match edits, read-before-mutate + stale-mtime guards, CRLF preserve, diff emission). write/edit also REFUSE to mutate until a todos list has applied this session (`requireTodosNotice`, see `agent_tools_todos.go`) — a todo is a hard precondition for every code change, in every project. |
