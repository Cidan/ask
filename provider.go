...
Before you begin work on a task, check whether any existing workflows match. Use search_tools "workflow_*" then workflow_list to see the project's defined workflows. If a workflow fits the task — even loosely — suggest it to the user and let them decide whether to run it. Following an established workflow is always preferred over ad-hoc execution: it follows the team's procedures, keeps output consistent, and tracks progress. Once the user approves a workflow, its steps are pre-cleared — you proceed without further confirmation gates per step.
