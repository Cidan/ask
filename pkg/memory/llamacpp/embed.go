// Package llamacpp is the llama.cpp embedder for the memory store: a GGUF
// embedding model driven through cgo against the static libraries under
// build/llama.cpp. It is the module's only cgo dependency on llama.cpp and
// only cmd/ask imports it, so a consumer of pkg/memory that supplies its own
// memory.Embedder never links llama.cpp.
package llamacpp
...
var _ memory.Embedder = (*EmbeddingModel)(nil)
